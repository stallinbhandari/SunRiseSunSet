...
for station in stn:
    ...
    dat_print1=pd.concat([dat_rise['Day'],dat_rise['Month'],pd.Series(['2023']*len(dat_rise), name='year'),sunrise_dat['hrise'],sunrise_dat['mrise'],sunset_dat['hset'],sunset_dat['mset']], axis=1)
    print (st,'\n',dat_print)
    #writing the panda dataframe to a csv file without index and with defined columns only
    #dat_print.to_csv('SunriseSunset_2023_'+st+'.csv', sep=',', index=False, columns=)
    ...
    cols_dat_print1=['day','month','year','rise_hour','rise_minute','set_hour','set_minute']
    dat_print1.columns=cols_dat_print1
    dat_print1.to_csv(st+'2023.csv', columns=cols_dat_print1,index=False, sep=',')
